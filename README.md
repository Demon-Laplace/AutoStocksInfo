...
2. 在 Authentication 中创建或邀请你自己的用户。
...
migration 位于 `supabase/migrations/0001_initial_schema.sql`，会创建 `holdings`、`related_assets`、`news_items`、`sec_filings`、`daily_reports`、`price_snapshots`，并为所有表启用 RLS。
...
前端使用 Supabase magic link 登录。数据读取依赖 Supabase Auth 和 RLS，只能读取当前登录用户自己的数据。
...
- 所有业务表已启用 RLS。
- authenticated 用户的 select/insert/update/delete policy 都限制为 `auth.uid() = user_id`。
